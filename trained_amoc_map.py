...
This script visualizes spatial patterns of PCA-derived feature importance for 
the Atlantic Meridional Overturning Circulation (AMOC) machine learning model. 

Main functionalities:
- Loads saved ML input/output data and PCA contribution arrays.
- Loads oceanographic datasets: Sea Surface Temperature (SST), Sea Surface Salinity (SSS), 
  and Sea Surface Height (SSH), and Runoff.
- Rebuilds the final valid spatial mask to match PCA feature contributions.
- Maps the relative importance of PCA components spatially on latitude-longitude grids 
  using Cartopy for SST, SSS, and SSH separately.
- Saves high-resolution PNG figures of the spatial importance maps for further analysis 
  and publication.

This visualization helps interpret which spatial regions and variables most influence 
the trained AMOC weakening prediction model.

Author: Farshid Daryabor  
Date: 2025-08-04

Usage:
    python trained_amoc_map.py

Requirements:
    - numpy, matplotlib, xarray, cartopy
    - PCA contribution data and ML input/output data files generated by the AMOC ML pipeline
"""
...
for name, start, size in zip(block_names, block_starts, block_sizes):
    ...
    if name == "runoff":
        ...
        ax = plt.gca()
        im = plt.imshow(contrib_map, cmap="viridis", interpolation='nearest')
        plt.title("PCA Contribution Map: RUNOFF", fontsize=14)
        plt.xlabel("X")
        plt.ylabel("Y")
