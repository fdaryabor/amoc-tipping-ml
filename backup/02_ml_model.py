...
This script is the second stage in the AMOC tipping point machine learning pipeline.
It builds upon preprocessed data from `01_ml_model.py` and a fitted PCA model from `apply_pca.py`.

Main Objectives:
---------------
1. **Apply PCA Transformation**  
   - Load `X_train_scaled.pkl` and `X_test_scaled.pkl`
   - Apply the fitted PCA model (`pca_model.pkl`) to reduce dimensionality
   - Save PCA-transformed data and create mini-batches for efficiency

2. **Balance the Training Set with SMOTE**  
   - Address class imbalance using Synthetic Minority Over-sampling Technique
   - Save the balanced training features and labels

3. **Train and Evaluate ML Models**  
   - Train Random Forest and XGBoost classifiers on balanced, PCA-reduced features
   - Evaluate model performance via classification reports, ROC, and PR curves

4. **Feature Importance Analysis**  
   - Reconstruct contribution of original oceanographic blocks (e.g., SST, SSH) from PCA components
   - Save and visualize relative importance per variable block

5. **SHAP Explainability**  
   - Use SHAP to explain XGBoost model predictions
   - Save SHAP summary plot for model interpretability

6. **Save Final Data for Reproducibility**  
   - Bundle all important inputs/outputs into a single `.npz` file
   - Enables consistent reuse in downstream analysis or publications

Requirements:
-------------
- Must run `apply_pca.py` before executing this script
- Input files: 
    - `X_train_scaled.pkl`, `X_test_scaled.pkl`, `y_train.pkl`, `y_test.pkl`
    - `pca_model.pkl` (generated in `apply_pca.py`)
    - `sst_clean.pkl`, `sss_clean.pkl`, `ssh_clean.pkl`, `X_runoff.pkl`, `X.pkl`
    - `preprocessed_dims.npz`, `valid_mask_combined.npy`, `sst_times_norm.npy`, `valid_time_mask2.npy`

Outputs:
--------
- Trained ML models: `trained_amoc_model.pkl`
- Evaluation plots: `figures/roc_curve.png`, `figures/precision_recall_curve.png`
- Feature importance results: `pca_contrib_data.npz`, `figures/Feature_Importance_by_Block.png`
- SHAP explanations: `figures/shap_summary.png`
- Final data bundle: `amoc_ml_input_output.npz`
